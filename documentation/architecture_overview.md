...
4.  **Stateless Multi-Tenancy (Demo Mode)**: To allow multiple recruiters to test the live demo simultaneously without seeing each other's data, the app uses stateless multi-tenancy. The React frontend silently generates a unique `localStorage` session ID and passes it via an `X-Tenant-ID` header using Axios interceptors. The Node.js server scopes every single MySQL operation (`SELECT`, `INSERT`, `UPDATE`, `DELETE`) to this ID, creating perfectly isolated data workspaces without requiring a heavy authentication system.
5.  **Keep-Alive Cron Strategy**: Cloud providers (like Render) spin down free-tier instances after 15 minutes of inactivity, causing massive 60-second cold start delays for visitors. To bypass this, the project utilizes a GitHub Actions automated workflow (`.github/workflows/keep-alive.yml`) that fires an HTTP `GET /health` request every 10 minutes. This explicitly prevents the backend instances from sleeping, guaranteeing instant performance for recruiters reviewing the portfolio.
